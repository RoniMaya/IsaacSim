from isaacsim import SimulationApp



simulation_app = SimulationApp({"headless": True, "enable_cameras": True})
from pxr import UsdLux, Gf
from isaacsim.core.prims import RigidPrim

import numpy as np
import io
import time
import omni
import carb
from PIL import Image
from fastapi import FastAPI
from starlette.responses import StreamingResponse
import threading
import omni.isaac.core.utils.numpy.rotations as rot_utils
from omni.isaac.core.utils.prims import set_prim_property

from omni.isaac.core import World
from omni.isaac.core.objects import DynamicCuboid
from omni.isaac.sensor import Camera
import omni.kit.commands
from PIL import ImageDraw, ImageFont

# Isaac Sim setup
world = World()
world.scene.add_default_ground_plane()
world.reset()
stage = omni.usd.get_context().get_stage()


# Add a cube to the scene
cube = world.scene.add(
    DynamicCuboid(
        prim_path="/World/cube",
        size=1.0,
        position=np.array([0, 0, 0]),
        color=np.array([1.0, 0.0, 0.0])
    )
)
rigid_cube = RigidPrim("/World/cube")
pressed_keys = set()

# Add a camera looking at the cube
camera = Camera(
    prim_path="/World/StreamCamera",
    position=np.array([0, 0, 50]),
    frequency=20,
    resolution=(500, 500),
    orientation=rot_utils.euler_angles_to_quats(np.array([0, 90, 0]), degrees=True),
)
camera.initialize()
omni.kit.commands.execute('ChangeSetting',
    path='/rtx/scene/common/light/enableDefaultLight',
    value=False)
light_path = "/World/DomeLight"
dome_light = UsdLux.DomeLight.Define(stage, light_path)
dome_light.GetColorAttr().Set(Gf.Vec3f(1.0, 1.0, 1.0))
dome_light.GetIntensityAttr().Set(1000.0)



# light_path = "/World/SphereLight"
# light_prim = UsdLux.SphereLight.Define(stage,light_path)


# # get prim and its attribute ("intensity")
# attribute = light_prim.GetIntensityAttr()
# attribute.Set(3000000)  # change attribute


def keyboard_event(event, *args, **kwargs):
    global pressed_keys
    if event.type == carb.input.KeyboardEventType.KEY_PRESS:
        pressed_keys.add(event.input)
    elif event.type == carb.input.KeyboardEventType.KEY_RELEASE:
        if event.input in pressed_keys:
            pressed_keys.remove(event.input)   


latest_jpeg = None  # Global shared frame buffer

# MJPEG generator
def generate_frames():
    global latest_jpeg
    while True:
        if latest_jpeg:
            yield (
                b"--frame\r\n"
                b"Content-Type: image/jpeg\r\n\r\n" +
                latest_jpeg + b"\r\n"
            )
        time.sleep(1 / 20)

direction = np.array([0.0, 0.0, 0.0])

# FastAPI server
app = FastAPI()

@app.get("/video")
def video_feed():
    return StreamingResponse(generate_frames(), media_type="multipart/x-mixed-replace; boundary=frame")
from fastapi import Body

@app.post("/control")
def update_control(command: str = Body(..., media_type="text/plain")):
    pressed_keys.add(command.upper())
    return {"status": "pressed"}

@app.post("/release")
def release_control(command: str = Body(..., media_type="text/plain")):
    pressed_keys.discard(command.upper())
    return {"status": "released"}

# Run FastAPI in a background thread

def start_fastapi():
    import uvicorn
    uvicorn.run(app, host="127.0.0.1", port=8000)

server_thread = threading.Thread(target=start_fastapi, daemon=True)
server_thread.start()

# Keep Isaac Sim alive
idx = 0
direction = np.array([0.0, 0.0, 0.0])
while simulation_app.is_running():
        world.step(render=True)
        frame = camera.get_rgba()

        if frame is None or len(frame.shape) != 3 or frame.shape[2] < 3:
            continue  # Wait for the next valid frame
        frame = frame[:, :, :3]  # Drop alpha channel
                # 1. Calculate the direction vector based on the keys currently in the set
        speed = 0
        if len(pressed_keys) > 0:
            direction = np.array([0.0, 0.0, 0.0])
            speed = 3
        if "A" in pressed_keys:
            direction += np.array([0.0, 1.0, 0.0]) # Forward
        if "D" in pressed_keys:
            direction += np.array([0.0, -1.0, 0.0]) # Backward
        if "W" in pressed_keys:
            direction += np.array([1.0, 0.0, 0.0]) # Left
        if "S" in pressed_keys:
            direction += np.array([-1.0, 0.0, 0.0]) # Right
        # if "+" in pressed_keys:
        #     speed += 1
        # if "-" in pressed_keys:
        #     speed -= 1


        # 2. Normalize the direction vector and scale by speed to get velocity
        #    We check if the magnitude is non-zero to avoid a division by zero error.
        norm = np.linalg.norm(direction)
        if norm > 0:
            normalized_direction = direction / norm
            velocity = normalized_direction * speed
        else:
            # If no movement keys are pressed, velocity is zero
            velocity = np.array([0.0, 0.0, 0.0])
        image = Image.fromarray(frame.astype(np.uint8), "RGB")

        # Draw velocity direction on image
        draw = ImageDraw.Draw(image)

        # Convert direction to string (rounded for readability)
        # velocity_str = f"V: [{velocity[0]:.2f}, {velocity[1]:.2f}, {velocity[2]:.2f}]"

        # Optional: pick font size
        try:
            font = ImageFont.truetype("DejaVuSansMono.ttf", 18)
        except:
            font = ImageFont.load_default()

        draw.text((10, 10), str(pressed_keys), font=font, fill=(255, 255, 0))  # Yellow text

        
        buffer = io.BytesIO()
        image.save(buffer, format="JPEG")
        latest_jpeg = buffer.getvalue()


        rigid_cube.set_linear_velocities(np.expand_dims(velocity, axis=0), indices=[0])
