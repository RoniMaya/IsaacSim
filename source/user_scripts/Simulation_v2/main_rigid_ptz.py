...
from DynamicAssets import ptz, ptz_rigid
...
# drive_base, drive_middle, camera_base = ptz_rigid.create_ptz_camera(enviorment.stage, prim_path = "/World/camera_prim", 
#                          translation = np.array([0,0,0]))


DynamicCuboid(prim_path="/World/camera_prim/base", color=np.array([0, 255, 0]))
DynamicCuboid(prim_path="/World/camera_prim/middle", color=np.array([255, 0, 0]))
...
while simulation_app.is_running():
    if world.is_playing():
        ...
        rot_flag = controller.rot_flag(mapping, 'car2')
        print(rot_flag)

        #----------------------------------------------------------------------------
        # set the velocity, orientation and zoom of the "camera" cube
        translation, orientation = cube.get_position_and_orientation()
        cube.set_angular_velocity([camera_orientation], orientation)
        cube.set_velocity(velocity, orientation)
        camera.zoom_camera(zoom_factor)
        #-------------------------------------------------------------------------

        # update the velocity, orientation and zoom of the "camera" cube based on the pressed keys
        translation, orientation = camera_base.get_position_and_orientation()
        cube2_velocity = controller.update_velocity_direction(mapping, 'car2')
        # print(cube2_velocity)

        cube2_orientation = controller.update_orientation(mapping, 'cube2')
        #----------------------------------------------------------------------------

        # set the velocity, orientation and zoom of the "camera" cube
        cube2.set_angular_velocity([cube2_orientation], orientation)
        camera_base.set_velocity(cube2_velocity, orientation)
        velocity = camera_base.get_linear_velocity(orientation)
        # print(velocity)
